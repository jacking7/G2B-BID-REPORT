...
- For production-only smoke tests and health checks, prefer the domain `g2b-report.duckdns.org` over the raw server IP unless the task explicitly needs IP-level diagnosis. Keep local/development tests on localhost or the active local dev server URL.
...
## Response style
- Format user-facing answers for a Codex CLI terminal using a Dracula-like theme.
- Do not force HTML colors; rely on Markdown that terminals can highlight naturally.
- Use short headings, bullets, inline code for file paths, commands, URLs, commits, system values, and identifiers, and fenced code blocks for logs, command examples, config, and real code.
- Keep code/system details visually separated from prose so they are easy to scan in a terminal.
